...
module.exports = function () {
  ...
  argv
    ...
    .option('-e, --elasticsearchHost <url>', 'elasticsearch url')
    ...
    .option('--rtailPort <port>', 'forward logs to rtail-server with given udp port')
    .option('--rtailHost <hostname>', 'hostname to forward logs to rtail-server')
    .option('--rtailWebPort <port>', 'starts rtail UI webserver (if installed) - npm i rtail -g)')
    .option('--rtailWebHost <hostname>', "rtail UI webserver and bind hostname\n\t\t\t\tExample: logagent --rtail-web-port 9000 --rtail-port 8989  --rtail-web-host $(hostname) -g '/var/log/**/*.log'")
    .parse(process.argv)
