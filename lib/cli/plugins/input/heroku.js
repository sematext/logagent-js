  ...
  try {
    ...
    req.on('end', function () {
      ...
      lines.forEach(function (line) {
        ...
        eventEmitter.emit('data.raw', line, {sourceName: 'heroku_' + token, index: token, filter: filterHerokuMessage})
      })
      res.end('ok\n')
    })
  } catch (err) {
...
InputHeroku.prototype.startHerokuServer = function () {
  this.getHttpServer(Number(this.config.heroku), this.herokuHandler)
