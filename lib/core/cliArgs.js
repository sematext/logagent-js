...
function writeConfig (path) {
  try {
    ...
    var cfg = fs.readFileSync(require.resolve('./../../config/example.yml')).toString()
    if (argv.index) {
    ...
    if (argv.elasticsearchUrl) {
      cfg = cfg.replace(/https:\/\/logsene-receiver.sematext.com/g, argv.elasticsearchUrl)
    }
...
function writePatterns (path) {
  try {
    ...
    var cfg = fs.readFileSync(require.resolve('./../../patterns.yml')).toString()
    fs.writeFileSync(path, cfg)
...
module.exports = (function () {
  ...
  argv
    ...
    .option('-w, --writeConfig <file>', 'write example config to a file, using -i,-e,-g args in generated config')
    ...
    .option('--geoipEnabled <value>', 'true/false to enable/disable geoip lookups, default field "client_ip"')
    .option('--geoipFields <value>', 'comma separated list of fields for geoip lookups')
    ...
    .option('--docker <docker-socket>', 'path to docker socket for container log collection')
    ...
    .option('--journald <port>', 'starts http server to receive logs systemd-journal-upload.service')
    .option('--writePatterns <file>', 'write example patterns.yml to a file', writePatterns)
    .parse(process.argv)
