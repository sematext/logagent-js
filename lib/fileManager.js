...
FileManager.prototype.getTailPosition = function (file) {
  ...
  if (!storedPos) {
    debug('no position stored for ' + file)
    ...
    if (stat.ino === storedPos.inode) {
      ...
      debug('Watching file ' + file + ' inode changed, set tail position = 0')
      return {start: 0}
    }
  }
}
...
FileManager.prototype.tailFile = function (file) {
  ...
  try {
    ...
    log('Watching file:' + file + ' from position: ' + pos.start)
    return tail
  } catch (error) {
    //log('ERROR tailing file ' + file + ': ' + error)
    return null
...
FileManager.prototype.terminate = function () {
  var filePositions = this.filesToWatch.map(function (tailObj) {
    ...
    log('unwatch ' + tailObj.filename + ' inode: ' + position.inode + ' pos:' + position.pos)
    return position
  })
  try {
