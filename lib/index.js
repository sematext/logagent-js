...
function LogParser (fileName, options, readyCallback) {
  ...
  if (!this.options.whitelist) {
    this.options.whitelist = (process.env.LOGAGENT_PATTERN_WHITELIST || ' ').split(',')
  }
  if (!this.options.blacklist) {
    this.options.blacklist = (process.env.LOGAGENT_PATTERN_BLACKLIST || ' ').split(',')
  }*/
  this.sources = {}
...
LogParser.prototype = {
  ...
  load: function (name, options) {
    try {
      ...
      if (e.reason && e.mark) {
        ...
        console.log('Error parsing file: ' + name + ' ' + e + ' ' + e.stack)
      }
      this.patterns = []
      process.exit()
    } finally {
  ...
  matchPatterns: function (p, parsed, line) {
    ...
    if (match) {
      if (p.inputFilter && p.inputFilter.test) {
        try {
          ...
          console.log('Error in' + p.type + '.dropMessageFilter.test():' + ex)
        }
      }
      parsed._type = p.type
      if (p.fields && (match.length > p.fields.length)) {
        for (var i = 0; i < p.fields.length; i++) {
          ...
          // fieldDefinition = p.fields[i].split(':')
          // [0] Field Name
          // [1] Field Type
  ...
  _parseLine: function (line, source, cbf) {
    ...
    if (this.patternMatchingDisabled || this.sourceBlacklist[source] >= this.MAX_TRAINING_LINES) {
      ...
      return cbf('pattern matching disabled for this logSource: ' + source, rv)
    }
