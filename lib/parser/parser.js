...
LogParser.prototype = {
  ...
  enrichGeoIp: function (parsedObject, fieldName) {
    if (geoipCity != null && parsedObject[fieldName]) {
      ...
      if (location && location.country && location.city && location.city.names && location.city.names.en) {
        parsedObject['geoip'] = {location: [location.location.longitude, location.location.latitude], info: {
  ...
  getPatternsForSource: function (sourceName) {
    ...
    this.sources[sourceName].patterns = include
    return include
  },
  ...
  parseDate: function (strDate, dateFormat) {
    ...
    if (dateFormat && strDate) {
      var dateStrRmDoubleSpaces = strDate.replace(TWO_SPACES, SPACE)
      d = moment(dateStrRmDoubleSpaces, dateFormat, false)
      if (d && !d.isValid()) {
        d = moment(dateStrRmDoubleSpaces, this.cfg.dateFormats, true)
      }
    }
    if (d && d.isValid()) {
      return d.toDate()
  ...
  parseJson: function (line, source) {
    ...
    if (/^\[{0,1}\{.*\}\]{0,1}$/.test(line)) {
      ...
      if (parsed.msg && parsed.time && parsed.v && parsed.pid && parsed.level) {
        // bunyan format
        parsed.message = parsed.msg
        delete parsed.msg
        parsed['@timestamp'] = new Date(parsed.time)
        parsed.level = String(parsed.level)
        parsed.severity = bunyanLogLevels[parsed.level] || 'info'
      }
      if (this.cfg.json != undefined && this.cfg.json.transform != undefined) {
        this.cfg.json.transform.call(null, source, parsed, this.cfg.json)
      }
  ...
  parseLine: function (line, source, cbf) {
    ...
    function mlReaderCb (data) {
      setImmediate(function parseLineWrapper () {
        call(self._parseLine.bind(self, data, source, cbf),
          parseResultHandler.bind({cbf: cbf}))
      })
    }
