...
LogParser.prototype = {
  ...
  hash: function hash (input) {
    ...
    console.log(this.cfg.hashFunction)
  ...
  parseDate: function (strDate, dateFormat) {
    ...
    if (dateFormat && strDate) {
      var dateStrRmDoubleSpaces = String(strDate).replace('  ', ' ')
      d = moment(dateStrRmDoubleSpaces, dateFormat.trim(), true) || moment(dateStrRmDoubleSpaces, this.cfg.dateFormats, true)
    }
    if (d && d.isValid()) {
      return d.toDate()
    } else {
  ...
  matchPatterns: function (p, parsed, line) {
    ...
    if (match) {
      ...
      if (p.fields && (match.length > p.fields.length)) {
        ...
        if (p.transform) {
          call(p.transform.bind(null, parsed), resultTransform.bind({name: p.type}))
        }
        if (p.filter) {
