...
InputAzureEventhub.prototype.startAsync = async function () {
  var context = {
    ...
    sourceName: this.config.sourceName || this.config.name || 'input.azure.eventhub',
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      if (events[i].body && events[i].body.records) {
        for (var k = 0; k < events[i].body.records.length; k++) {
          ...
          eventEmitter.emit('data.raw', JSON.stringify(tmp), context)
        }
      } else {
        eventEmitter.emit('data.raw', JSON.stringify(msg), context)
      }
    }
  }
  ...
  for (var p = 0; p < partitionIds.length; p++) {
    this.subscriptions.push(
      this.client.subscribe(partitionIds[p], subscriberHandlers, subscriberOptions)
    )
  }
}
