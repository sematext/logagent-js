...
InputCommand.prototype.start = function () {
  if (!this.started) {
    ...
    if (/\$QUERY_TIME/.test(this.config.command)) {
      ...
      this.lastQueryTimeFile = this.config.lastQueryTimeFile || 
                               path.join(os.tmpdir(), 'logagentLastQueryTimeFile')
      // by default, query this century
      var defaultLastRun = this.config.initialQueryTime || "2001-01-01T00:00:00"
      this.lastRun = moment(defaultLastRun, moment.ISO_8601)
                          .format(dateFormat)
      try {
        this.lastRun = moment(fs.readFileSync(this.lastQueryTimeFile), dateFormat);
      } catch (err) {
        logger.warn("Error while getting the date from" 
          + this.lastQueryTimeFile + ". Using " + moment(this.lastRun).format(dateFormat)
          + ". We'll try to create that file on each query. Error was: " + err)
      }
    }
    // default buffer for the command pipe is 50MB
    var maxBuffer = this.config.maxBuffer || 50000000
    this.runCommand(this.config.command, {
      sourceName: this.config.sourceName || this.config.command
    }, dateFormat, maxBuffer)
  }
}
...
InputCommand.prototype.runCommand = function (cmd, context, dateFormat, maxBuffer) {
  var self = this
  var exec = require('child_process').exec
  var cmdTemplate = cmd.replace(/\$NOW/g, moment().format(dateFormat))
  if (this.config.QUERY_TIME) {
    cmdTemplate = cmdTemplate.replace(/\$QUERY_TIME/g, moment(self.lastRun).format(dateFormat))
  }
  
  logger.debug(cmdTemplate)
  var child = exec(cmdTemplate, {"maxBuffer": maxBuffer})
  ...
  if (this.config.QUERY_TIME) {
    try {
      fs.writeFileSync(this.lastQueryTimeFile, moment(self.lastRun).format(dateFormat))
    } catch (err) {
