...
function InputDockerSocket (config, eventEmitter) {
  ...
  this.opts = {
    ...
    skipByImage: process.env.SKIP_BY_IMAGE || config.SKIP_BY_IMAGE || 'sematext/.*agent.*',
    attachFilter: function (id, info) {
      ...
      if (LOGS_ENABLED_DEFAULT === false &&
        ...
        dockerInfo.LOGSENE_ENABLED = true
        return true
      } else {
  ...
  eventEmitter.on('dropLogsRequest', function (context, data) {
    if (self.lh) {
      if (self.config.debug) {
        consoleLogger.log(`Docker dropLogsRequest: disable log stream for ${context.container_name} ${context.container_id}`)
      }
  ...
  eventEmitter.on('collectLogsRequest', function (context, data) {
    ...
    if (self.lh && self.lh.attachContainer) {
      if (self.config.debug) {
        consoleLogger.log(`collectLogsRequest: enable log stream for ${context.container_name} ${context.container_id}`)
      }
