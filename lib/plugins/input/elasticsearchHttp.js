...
InputElasticsearchHttp.prototype.validateRequest = function (req, res) {
  ...
  if (
    /\/_nodes|\/_search|\/_cat|\/_count|\/_settings|\/_mapping|\/_aliases|\/_reindex|\/_cluster/.test(
      req.url
    )
  ) {
...
InputElasticsearchHttp.prototype.elasticSearchHttpHandler = function (
  req,
  res
) {
  try {
      ...
      document.forEach(function (line) {
        if (isJson(document[offSet])) {
          ...
          if (lineObj.index) {
            ...
            var emitMsg = createIndexCall(
              lineObj.index,
              source,
              reqInfo.defaultIndex,
              reqInfo.defaultType
            )
            var responseItem = {
              index: {
            ...
            self.eventEmitter.emit('data.object', emitMsg, {
              source: 'input-elasticsearch-http',
              index: lineObj.index._index
            })
          } else {
...
InputElasticsearchHttp.prototype.startElasticsearchHttp = function (id) {
  this.getHttpServer(
    ...
    this.elasticSearchHttpHandler.bind(this)
  )
  ...
  var terminate = function (reason) {
    return function () {
      ...
      exitInProgress = true
      consoleLogger.log(
        'Stop Elasticsearch http worker: ' +
          id +
          ', pid:' +
          process.pid +
          ', terminate reason: ' +
