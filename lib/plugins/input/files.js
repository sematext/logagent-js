...
InputFile.prototype.start = function () {
  ...
  if (globPattern) {
    ...
    this.tailFilesFromGlob(globPattern, 10000)
  }
}
...
InputFile.prototype.getTempDir = function () {
  return (
    this.options.diskBufferDir || process.env.LOGSENE_TMP_DIR || os.tmpdir()
  )
}
...
InputFile.prototype.getTailPosition = function (file) {
  ...
  if (this.options.tailStartPosition !== undefined || !storedPos) {
    ...
    // tail from end of file or specified start position ("--tailStartPosition")
    var pos = { start: getFilesizeInBytes(file) }
    if (
      this.options.tailStartPosition !== undefined &&
      !isNaN(tailStartPosition)
    ) {
      if (tailStartPosition >= 0) {
        pos.start = tailStartPosition
      } else {
        // negative tailStartPosition values: read from File.size - N bytes
    ...
    if (stat.ino === storedPos.inode) {
      ...
      logger.debug(
        'Watching file ' + file + ' inode changed, set tail position = 0'
      )
      return { start: 0 }
    }
  }
    ...
    glob(
      ...
      function globCb (err, files) {
        if (!err) {
          ...
          logger.error(
            'Error in glob file patttern ' + globPattern + ': ' + err.message
          )
        }
      }.bind(this)
    )
  ...
  try {
    ...
    tail.on(
      ...
      function (error) {
        ...
        this.eventEmitter.emit('error', errMessage, {
          file: file,
          error: error
        })
      }.bind(this)
