...
function InputFile (options, eventEmitter) {
  ...
  this.timers = []
  var self = this
  // stop all tail-forever instances every 10 minutes and restart watching
  // this should release open file descriptors in tail-forver
  setInterval(self.resetFilePointers.bind(this), 10 * 60 * 1000)
}
...
InputFile.prototype.start = function () {
  this.timers.forEach(function stopTimer (t) {
    clearInterval(t)
  })
...
InputFile.prototype.tailFilesFromGlob = function (globPattern, scanTime) {
  if (globPattern) {
    ...
    if (!this.globPattern && scanTime > 0) {
      ...
      var tid = setInterval(function scanFilesTimer () {
        this.scanCounter = 1
...
InputFile.prototype.resetFilePointers = function () {
  logger.log('reset file pointers')
  var self = this
  self.savePositions()
  self.filePointers = self.readFilePointers()
  self.scanCounter = 0
  // var ftw = self.filesToWatch
  self.filesToWatch = []
  self.fileNamesToWatch = []
  self.start()
/*
ftw.forEach(function startTailFile (tailObj) {
  try {
    self.tailFile(tailObj.filename)
  } catch (error) {
    logger.log(error)
  }
})
