...
class Parser {
  ...
  consume (data) {
    ...
    if (/^__CURSOR=/.test(data)) {
      this.endMultiLineField()
    }
    // regular field: a=b
    if (/^\S+=.+$/.test(data) && data.length > 0 && this.isMultilineMode() === false) {
      this.addField(data)
      return
    ...
    if (this.isMultilineMode() && data.length === 0 && this.multiLineFieldValue.length > 3) {
      this.endMultiLineField()
      return
    }
...
JournaldUpload.prototype.parseBody2 = function (body, token) {
...
JournaldUpload.prototype.parseBody = function (body, token) {
  // console.log(body.match(exportRecordRegex.match()))
  // this.emitEvent(log, token)
}
...
JournaldUpload.prototype.journaldHttpHandler = function (req, res) {
  try {
    ...
    const parserState = new Parser(self.emitEvent.bind(self))
    req.pipe(createStreamThrottle(this.throughputPerClient)).pipe(split())
      .on('data', parserState.consume.bind(parserState))
      .on('end', function endHandler () {
        self.emitEvent(parserState.end(), token)
