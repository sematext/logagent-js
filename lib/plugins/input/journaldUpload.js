...
function JournaldUpload (config, eventEmitter) {
  ...
  eventEmitter.on('error', function (err) {
    ...
    if (match && match.length > 1) {
      ...
      self.invalidTokens[match[1]] = true
    }
  })
  if (!self.config.cleanBlacklistTime) {
    self.config.cleanBlacklistTime = 5 // minutes
  }
  // drop blacklist every 10 minutes 
  // just in case apps get enabled or accessed too early 
  setInterval(function () {
    self.invalidTokens = {}
  }, Math.max(self.config.cleanBlacklistTime, 1) * 60 * 1000)
}
...
JournaldUpload.prototype.journaldHttpHandler = function (req, res) {
  try {
    ...
    if ((self.config.useIndexFromUrlPath === true && !token) || self.invalidTokens[token])  {
      res.statusCode = 401
