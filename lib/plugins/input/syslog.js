...
InputSyslog.prototype.start = function () {
  if (this.config.udp) {
    try {
      ...
      syslogd.listen(this.config.udp, function (err) {
        consoleLogger.log('start syslog server ' + syslogd.server.address().address + ':' + this.config.udp + ' ' + (err || ''))
      }.bind(this))
    } catch (err) {
      console.log(err)
    }
...
module.exports.plugin = InputSyslog
