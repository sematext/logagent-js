...
UnixSocketReader.prototype.start = function () {
  ...
  function errorHandler (error) {
    if (error) {
      consoleLogger.error(`Error connectiong to ${self.config.path}: ${error.message}`)
    }
  }
  ...
  if (!this.started) {
    ...
    this.client.on('connect', function (err) {
      if (!err) {
        ...
        self.client.pipe(Throttle(self.config.maxInputRate)).pipe(split()).on('data', function emitLine (data) {
        ...
        consoleLogger.error(`Error connectiong to ${self.config.path}: ${err.message}`)
      }
    })
    this.started = true
  }
