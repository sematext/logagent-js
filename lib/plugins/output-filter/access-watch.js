...
function init (config, context, eventEmitter) {
  ...
  if (!config.apiKey) {
    console.log('Please configure the Access Watch plugin with an apiKey: get one for free from https://access.watch/reveal')
    return
  }
...
function fetchIdentityData (address, userAgent, config, data, callback) {
  ...
  requestData(options, function (result) {
    if (result) {
      data = augmentData(data, result, config['identityDestination'], config['identityProperties'])
      ...
      if (result.hasOwnProperty('reputation') && config.hasOwnProperty('reputationDestination')) {
        data = augmentData(data, result['reputation'], config['reputationDestination'], config['reputationProperties'])
      }
    }
    callback(null, data)
  })
...
function accessWatch (context, config, eventEmitter, data, callback) {
  ...
  if (config.matchTypes && data._type && config.matchTypes.indexOf(data._type) > -1) {
    ...
    if (config.addressField && data[config.addressField]) {
      address = data[config.addressField]
    }
