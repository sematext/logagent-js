...
function parseCloudFoundryTags (tags) {
  console.log(tags)
  try {
    const splitTagsArr = tags.split(' ')
    const parsedTags2dArr = splitTagsArr.map(t => t.replace(/"/g, '').split('='))
    const parsedTags = parsedTags2dArr.map(a => {
      const key = a.shift()
      const val = a.shift()
      const obj = {}
      obj[key] = val
    ...
    const reducedTags = parsedTags.reduce((acc, o) => {
