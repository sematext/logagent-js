...
module.exports = function enrichDockerLogs (context, config, eventEmitter, data, cb) {
  ...
  var imageName = context.image || ''
  var imageVersion = ''
  var imageProperties = imageName.split(splitImageRegex)
  // removing leading registry
  var matchResult = imageName.match(imageHostPortRegex)
  if (matchResult) {
    // image name containes registry host:port
    // remove registry name from imageName
    if (matchResult[1]) {
      imageName = imageName.substring(matchResult[1].length, imageName.length)
    }
  }
  var imageProperties = imageName.split(splitImageRegex)
  if (imageProperties.length >= 2) {
    imageName = imageProperties[0]
    imageVersion = imageProperties.slice(1).join(':')
  }
  data.container = {
