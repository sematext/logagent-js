...
function initMaxmind (cb) {
  var fileName = (process.env.MAXMIND_DB_DIR || geoIpStatus.maxmindDbDir) + 'GeoLite2-City.mmdb'
  geoIpStatus.maxmindUpdate = maxmindUpdate(true, (process.env.MAXMIND_DB_DIR || geoIpStatus.maxmindDbDir), function (err, filename) {
    initGeoIp(err, filename, cb)
  })
}

function geoIpEnrichment (context, config, eventEmitter, data, callback) {
  if (config.geoIpFailed === true) {
    return callback(null, data)
  }
  // buffer logs having relevant fields until GeoIp DB is ready
  if (!config.geoIpReady) {
  ...
  if (!config.geoIpInit) {
    config.geoIpInit = true
    if (!geoIpStatus) {
      geoIpStatus = config
    }
    initMaxmind(function (err) {
      if (!err) {
        config.geoIpReady = true
      } else {
        config.geoIpFailed = false
        consoleLogger.error('Error initializing Geo IP module ' + err)
      }
