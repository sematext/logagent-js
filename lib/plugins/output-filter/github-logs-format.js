...
const parseRelease = (event, body) => {
  ...
  return {
    ...initEvent({ event, action, webhookName: 'GitHub' }),
    message: `${initRepoMessage({ repoName: parsedRepo.name, repoUrl: parsedRepo.url })} ${event} ${parsedRelease.tag} - ${parsedRelease.url} ${action} by ${initAuthorMessage({ senderUrl: parsedSender.url, senderName: parsedSender.username })}`,
    event,
    action,
    repository: parsedRepo,
    sender: parsedSender,
...
const parseCreate = (event, body) => {
  ...
  return {
    ...initEvent({ event, action, webhookName: 'GitHub' }),
    message: `${initRepoMessage({ repoName: parsedRepo.name, repoUrl: parsedRepo.url })} Branch ${parsedBranch.name} - ${parsedBranch.url}) created by ${initAuthorMessage({ senderUrl: parsedSender.url, senderName: parsedSender.username })}`,
    event,
    action,
    repository: parsedRepo,
    sender: parsedSender,
...
const parseDelete = (event, body) => {
  ...
  return {
    ...initEvent({ event, action, webhookName: 'GitHub' }),
    message: `${initRepoMessage({ repoName: parsedRepo.name, repoUrl: parsedRepo.url })} Branch ${parsedBranch.name} - ${parsedBranch.url}) deleted by ${initAuthorMessage({ senderUrl: parsedSender.url, senderName: parsedSender.username })}`,
    event,
    action,
    repository: parsedRepo,
    sender: parsedSender,
