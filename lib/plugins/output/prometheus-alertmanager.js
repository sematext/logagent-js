...
OutputPrometheusAlertmanager.prototype.buildAlert = function (data, context) {
  ...
  if (this.config.alertTemplate) {
    ...
    for (var i = 0; i < labels.length; i++) {
      var label = labels[i]
      alert.labels[label] = format(
        this.config.alertTemplate.labels[label],
        data
      )
    }
    ...
    for (var j = 0; j < annotations.length; j++) {
      ...
      alert.annotations[annotation] = format(
        this.config.alertTemplate.annotations[annotation],
        data
      )
    }
  }
    ...
    function responseHandler (error, response, body) {
      if (!error && response.statusCode < 300) {
        if (this.debug === true) {
          console.error(
            new Date(),
            'Alert sent to Prometheus Alertmanager: alert=' +
        ...
        console.error(
          new Date(),
          'Error while alerting Prometheus Alertmanager:',
          error,
          body
        )
      }
