...
function OutputRtail (config) {
  this.config = config
}

OutputRtail.prototype.rtailEventHandler = function (event, data, context) {
...
OutputRtail.prototype.start = function () {
  ...
  if (this.config['rtailPort']) {
    ...
    eventEmitter.on('data.parsed', this.rtailEventHandler.bind(this))
  }
}
