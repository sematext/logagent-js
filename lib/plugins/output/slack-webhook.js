    ...
    function responseHandler (error, response, body) {
      if (!error && response.statusCode < 300) {
        if (self.config.debug === true) {
          console.error(new Date(), 'Slack Message sent: ' + 'payload=' + JSON.stringify(payload), body)
        }
      } else {
        console.error(new Date(), 'Error in slack web-hook:', error, response.statusCode, body)
      }
    }
