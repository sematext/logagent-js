...
function logWithNodeModuleInfo (level, text) {
  ...
  stackman.callsites(e, function (err, callsites) {
    if (err) throw err
    var mi =
      path.basename(callsites[3].getFileName()) +
      ':' +
      callsites[3].getLineNumber() +
      ' '
    switch (level) {
      case ERROR:
...
function logToConsole (level, text) {
  if (process.env.DEBUG) {
    logWithNodeModuleInfo(level, 'v' + version + ' ' + text)
  } else {
    ...
    console.error(color, new Date().toISOString(), 'v' + version + ' ' + text)
  }
}
function log (text) {
