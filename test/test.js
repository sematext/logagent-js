      ...
      function (err, data) {
        if (err) {
          ...
          if (data.message === 'hello world' && data.counter === 1) {
              ...
              new Error(
                'message is wrong: ' + data['@timestamp'] + ' ' + data.message
              )
            )
          }
        }
...
describe('Logagent parse web server Log', function () {
  it('should return client_ip, status_code', function (done) {
    ...
    this.la = new Logagent(null, null, function ready (laReady) {
      ...
      laReady.parseLine(
        '91.67.80.14 - - [03/Apr/2016:06:25:38 +0000] "GET /about/ HTTP/1.1" 200 14243 "https://sematext.com/consulting/elasticsearch/" "Mozilla/5.0 (iPhone; CPU iPhone OS 8_1_1 like Mac OS X) AppleWebKit/600.1.4 (KHTML, like Gecko) Mobile/12B436 Twitter for iPhone"',
        'nginx',
        function (err, data) {
          if (err) {
            return done(err)
            ...
            if (
              data.message === 'GET /about/' &&
              data.client_ip === '91.67.80.14' &&
...
describe('Logagent parse unknown log', function () {
  it('should have message===logtext and timestamp', function (done) {
    ...
    la.parseLine(logText, 'nginx', function (err, data) {
      if (err !== 'not found') {
        done(err)
      } else {
        if (
